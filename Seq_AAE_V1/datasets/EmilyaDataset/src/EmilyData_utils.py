...
def get_hdf5_file_name(window_width,shift_step,sampling_interval):
    ...
    path_dataset = root_path+'Seq_AAE_V1/datasets/EmilyaDataset/'
...
def normalization_velocity(delta_X, max_velocity_vector = None, min_velocity_vector = None, return_max_min=False):
    X_new = []
    try:
        (max_velocity_vector.all() == None or min_velocity_vector.all() == None)
    except:
        (max_velocity_vector == None or min_velocity_vector == None)

    # or (max_velocity_vector == None or min_velocity_vector == None):  #! Changed this, might need .any() instead!
        X_concate = []
        for seq in delta_X:
