...
# import sys
# import os
# root_path = os.getcwd()[:-10]  # Gets the path above the root directory
# print root_path
...
class Double_GAN_Conditional_SAAE(BaseEstimator):
    ...
    def set_up_model(self):
        ...
        # This if statement does not get run, the else does however.
    ...
    def compute_loss_and_plot_latent_space(self,epoch,whichdataset='training'):
        ...
        metric_mse_v = K.eval(10000*(loss_autoencoder-metric_mse)/self.alpha)

        # loss_neg_entropy_encoder = (loss_neg_entropy_encoder / self.beta_classifier).eval()
        loss_neg_entropy_encoder = K.eval(loss_neg_entropy_encoder / self.beta_classifier)
    ...
    def training(self,dataset_obj):
        ...
        for epoch in range(self.max_epoch):
            ...
            # print "alpha = {}".format(self.alpha.eval())   # FIX THIS
            print "alpha = {}".format(K.eval(self.alpha))  # fixed?
            # Edited this: to make iterations a float.
            self.cur_lr_autoencoder = self.autoencoder_with_discriminator.optimizer.lr * (1. / (
                1. + self.autoencoder_with_discriminator.optimizer.decay * tf.to_float(self.autoencoder_with_discriminator.optimizer.iterations)))
            ...
            self.loss_history['beta_classifier'].append(K.eval(self.beta_classifier+0.0))
